...
<DIV class='content'>
  ...
  <DIV class='aside_menu'>
    <ASIDE style='float: left;'><A href='../category4/list.jsp'>게시판</A> > <A href='./list.jsp?categoryno=<%=categoryno %>'><%=category_title %></A>＞글 수정</ASIDE> 
    <ASIDE style='float: right;'>
   <A href='./list_category_table2.jsp?categoryno=<%=categoryno %>&col=<%=col%>&word=<%=word%>'>목록형</A> <span class='menu_divide'> |</span> 
      <A href='./list.jsp?categoryno=<%=categoryno %>&col=<%=col%>&word=<%=word%>'>앨범형</A> <span class='menu_divide'> |</span>    <A href='./list_read.jsp'>펼쳐보기</A>
  ...
  <DIV class='message'>
    ...
    if (pds4Proc.passwordCheck(pdsno, passwd) == 1) { // 패스워드 일치
      ...
      col = Tool.checkNull(request.getParameter("col"));
      word = Tool.checkNull(request.getParameter("word"));
  ...
  <DIV class='bottom_menu'>
    ...
    %> 
    <button type='button' onclick="location.href='./list.jsp?categoryno=<%=categoryno %>&pdsno=<%=pdsno %>&col=<%=col%>&word=<%=word%>'">목록</button>
  </DIV>
