...
word = Tool.checkNull(upload.getParameter("word"));
...
<DIV class='content'>
  ...
  <DIV class='aside_menu'>
    <ASIDE style='float: left;'><A href='../category4/list.jsp'>게시판</A> > <A href='./list.jsp?categoryno=<%=categoryno %>'><%=category_title %></A>＞파일 교체</ASIDE> 
...
if (pds4Proc.passwordCheck(pdsno, passwd) == 1) { // 패스워드 일치
  ...
  if (size1 > 0) {
    if (size1 < 10485760) { // 1 ~ 10485760 byte
      ...
      // 기존 파일 삭제
      // ------------------------------------------------
      Pds4VO oldPds4VO = pds4Proc.read(pdsno);
      Tool.deleteFile(upDir, oldPds4VO.getFstor1());
      Tool.deleteFile(upDir, oldPds4VO.getThumb());
      // ------------------------------------------------
...
%>
  ...
  <DIV class='bottom_menu'>
    ...
    %>
      <button type='button' onclick="location.href='./update_form.jsp?categoryno=<%=categoryno %>&pdsno=<%=pdsno %>&col=<%=col%>&word=<%=word%>&nowPage=<%=nowPage%>'">계속 수정 [글 수정화면으로 돌아가기]</button>
    <%  
    }
    %>   
    <button type='button' onclick="location.href='./list.jsp?categoryno=<%=categoryno %>&pdsno=<%=pdsno%>&col=<%=col%>&word=<%=word%>&nowPage=<%=nowPage%>'">목록</button>
  </DIV>
