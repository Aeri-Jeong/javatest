...
<DIV class='content'>  
 ...
 <DIV class='aside_menu'>
  ...
  <A href='../category4/list.jsp'>게시판</A> > <A href='./list.jsp?categoryno=<%=categoryno %>'><%=category_title %></A> </ASIDE> 
  <ASIDE style='float: right;'>
  ...
  for(int index=0; index < list.size(); index++){
    ...
    if (index != 0 && index % 4 == 0) {         // 4개의 이미지를 출력하고 
    %>
      <HR class='hr_dotted'>                        <%// 점찍고 %>
    <%  
    }
  %>
  <!-- 하나의 이미지 출력 -->
  <DIV class='div_grid_img'>
    ...
    if (pds4VO.getSize1() > 0) {
      if (thumb != null) {
      %> 
         <A href='./read.jsp?pdsno=<%=pdsno%>&categoryno=<%=categoryno %>&col=<%=col%>&word=<%=word%>&nowPage=<%=nowPage%>'></A>  <IMG src='./storage/<%=pds4VO.getThumb() %>' style='width: 100%; height: 150px;'>
        <br>
      <%
      } else {
      %>
    ...
    %>
      <!-- <DIV class='div_grid_text' style='height: 150px; width: 98%'>
        <DIV class='div_grid_text_content' style='text-align: center'>
          등록된 파일 없음
        </DIV>
      </DIV>
      -->
