...
<DIV class='content'>
  ...
  <fieldset class='fieldset_basic'>
    <legend class='legend_basic'>방명록 수정</legend>
      ...
      int count = 0;
      int cnt = visitorProc.passwordCheck(visitorno, passwd);
      if (cnt == 1) {    // 패스워드 일치
        count = visitorProc.update(visitorVO); // 처리된 레코드(수정된) 갯수
  ...
  <DIV class='bottom_menu'>
    <%
    if (count == 0) {       // 수정된게 없을 때
    %>
      <button type='button' onclick="location.href='./update_form.jsp?visitorno=<%=visitorno%>'">다시 수정 시도</button>  
  <%
  }
  %>
