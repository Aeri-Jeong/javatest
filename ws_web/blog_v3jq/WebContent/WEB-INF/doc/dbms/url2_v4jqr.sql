...
1) url2 테이블의 생성
   - 저장소 만들기
   - PRIMARY KEY(url2no): urlno컬럼의 값이 2번이상 등록되는 것을 차단.
...
부모테이블의 코드가 자식테이블에서 쓰이고 있으면 부모테이블 삭제가 안되므로 쓰이는 곳을 전부 찾아 삭제합니다.
이런 문제로 실무에서는 대부분 삭제 처리가 여러 절차를 거치지 않으면 불가능합니다.
데이터 관리를 위해 이처럼 삭제를 제한한 경우도 많습니다.
